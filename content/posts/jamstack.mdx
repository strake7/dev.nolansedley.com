---
title: Static Webpages and Jamstack
date: 2020-11-12
...
text and some pictures. For a site such, as this we should be able to just serve
...
would be to pre-render the html page earlier, at build time, rather than
compiling it repeatedly at runtime.

## Enter Jamstack

Of course, we could simply write some static html files and call it good but
there is sure to be a large amount of duplication and we might lose out on some
tooling with such an approach. Our inner-developer might cringe at such a
prospect. Fortunately the tooling for building pre-rendered static `sites has
progressed. Enter Jamstack, an architecture designed to solve this problem using
the modern tools and workflows web developers have come to love. With a static
site, files can be served directly from a CDN enabling the site to **easily
scale and cutting back on cost, complexity and risk**.
...
1. The site is pre-rendered, meaning web pages are generated at the time of
   build.
1. The pre-rendered site is deployed to a CDN where requests for pages simply
   return the already compiled HTML page.
...
Jamstack is a modern and fresh architecture for building pre-rendered sites. So
